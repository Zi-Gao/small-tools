 ...
 * @LastEditTime: 2021-02-12 21:22:29
 * @LastEditors: Zi_Gao
 * @Description: 多进制互相转换
		...
		while (true) {
			...
			while (true) {
				...
				if (inputDetection.intType(strInputNumType) == 1) {
					...
					continue;
				} else if (Integer.valueOf(strInputNumType).intValue() != 2
						&& Integer.valueOf(strInputNumType).intValue() != 8
						&& Integer.valueOf(strInputNumType).intValue() != 10
						&& Integer.valueOf(strInputNumType).intValue() != 16) {
					System.out.println("请输入 2,8,10,16 进制");
					continue;
				} else {
					break;
			...
			while (true) {
				...
				if (inputDetection.intType(strReturnNumType) == 1) {
					System.out.println("请输入 2,8,10,16 进制");
					continue;
				} else if (Integer.valueOf(strReturnNumType).intValue() != 2
						&& Integer.valueOf(strReturnNumType).intValue() != 8
						&& Integer.valueOf(strReturnNumType).intValue() != 10
						&& Integer.valueOf(strReturnNumType).intValue() != 16) {
					System.out.println("请输入 2,8,10,16 进制");
					continue;
				} else {
					break;
			...
			System.out.println(binaryConversion.Core(strInputNum, Integer.valueOf(strInputNumType),
					Integer.valueOf(strReturnNumType)));
			break;
		}
	}
}
